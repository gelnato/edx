...
Add the following to the __init__.py at the root of the path you want to
protect against using certain dependencies that should be forbidden::
    from openedx.tests.helpers import module_setup_teardown
    # pylint: disable=invalid-name
    setUpModule, tearDownModule = module_setup_teardown(forbidden_dependencies=['lms', 'cms'])
...
def module_setup_teardown(forbidden_dependencies):
    ...
    def setUpModule():  # pylint: disable=invalid-name
        ...
        for name in forbidden_dependencies:
            original_modules.update({m: mod for m, mod in sys.modules.iteritems() if m.startswith(name + '.')})
